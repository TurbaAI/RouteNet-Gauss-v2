...
#   exact-replay runs (metrics.json: sample_order != native)  MAPE within +-0.5 pt, R2 within +-0.02
...
def to_markdown(rows, curves, title):
    out = [f"## {title}", ""]
    out.append("| dataset | target | seed | mode | init | device | TF MAPE | torch MAPE | Δ MAPE | TF R² | torch R² | Δ R² | TF MAE µs | torch MAE µs | gate | passed |")
    out.append("|---|---|--:|---|---|---|--:|--:|--:|--:|--:|--:|--:|--:|---|---|")
    for r in rows:
        out.append(f"| {r['dataset']} | {r['target']} | {r['seed']} | {r['mode']} | {r['init']} | {r['device']} | {fmt(r.get('tf_mape'))} | {fmt(r['torch_mape'])} | "
...
def main():
    ...
    tf_cells = load_cells(os.path.join(args.tf, "results") if os.path.isdir(os.path.join(args.tf, "results")) else args.tf)
